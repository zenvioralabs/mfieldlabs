...
export default function Footer() {
  const openCapability = (event: React.MouseEvent<HTMLAnchorElement>, capability: string) => {
    event.preventDefault();
    window.history.pushState({}, "", `/?capability=${capability}#services`);
    window.dispatchEvent(new Event("capabilitychange"));
  ...
  return (
    <footer className="bg-navy px-5 py-12 text-white sm:px-8 md:px-12 lg:px-16">
      <div className="mx-auto grid max-w-[1160px] gap-10 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
        ...
        <div><p className="eyebrow mb-4 !text-sky">Capabilities</p>{capabilities.map(c=><a key={c.n} href={`/?capability=${c.n}#services`} onClick={(event) => openCapability(event, c.n)} className="block py-1 text-xs text-white/60 hover:text-white">{c.title}</a>)}</div>
