    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          className="modal-scroll fixed inset-0 z-[1000] overflow-y-auto bg-navy text-white"
        >
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
            <div className="absolute inset-0 opacity-35 [background-image:radial-gradient(circle,rgba(56,189,248,0.2)_1px,transparent_1px)] [background-size:24px_24px]" />
            <div className="absolute inset-y-0 right-0 w-full bg-gradient-to-l from-navy via-navy/95 to-navy/55 md:w-[65%]" />
          ...
          <button
            ...
            className="fixed right-5 top-5 z-20 flex h-11 w-11 items-center justify-center rounded-xl border border-white/15 bg-white/10 text-white backdrop-blur transition-colors hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky sm:right-8 sm:top-8"
          >
          ...
          <div className="relative mx-auto flex min-h-full w-full max-w-6xl items-center px-6 py-20 sm:px-10 sm:py-24">
            <div className="grid w-full items-center gap-10 md:grid-cols-[0.85fr_1.15fr] md:gap-12 lg:gap-16">
              <motion.div
                ...
                className="group relative mx-auto aspect-[4/5] w-full max-w-xs overflow-hidden rounded-2xl border border-white/10 shadow-2xl sm:max-w-md"
              >
                <Image
                  src={image}
                  alt=""
              ...
              >
                <div className="flex flex-col gap-2.5">
                  <h3 id={titleId} className="font-display text-3xl font-bold tracking-normal text-white sm:text-4xl">
                    {title}
                  </h3>
                  <p className="text-base font-medium leading-relaxed text-sky">{tagline}</p>
                </div>

                <p className="text-sm leading-relaxed text-white/65 sm:text-base">{body}</p>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {impactAreas.map((area) => (
                      <span
                        key={area}
                        className="rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-sky/60 hover:text-white"
                      >
                        {area}
                      </span>
                    ))}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <ul className="flex flex-col gap-2.5">
                    {outcomes.map((outcome) => (
                      <li key={outcome} className="flex items-start gap-2.5 text-sm leading-relaxed text-white/80">
